...
-- - Checkbox mengirim 'accepted'; yang belum disetujui disimpan NULL.
...
CREATE TABLE users (
    ...
    legal_agreement VARCHAR(8) CHARACTER SET ascii COLLATE ascii_bin NULL
        COMMENT 'Input legal_agreement: accepted jika dicentang; selain itu NULL',
    privacy_agreement VARCHAR(8) CHARACTER SET ascii COLLATE ascii_bin NULL
        COMMENT 'Input privacy_agreement: accepted jika dicentang; selain itu NULL',
    ...
    CONSTRAINT users_legal_agreement_valid CHECK (
        legal_agreement IS NULL OR legal_agreement = 'accepted'
    ),
    CONSTRAINT users_privacy_agreement_valid CHECK (
        privacy_agreement IS NULL OR privacy_agreement = 'accepted'
    ),
    CONSTRAINT users_complete_profile_required_fields CHECK (
        profile_completed_at IS NULL OR (
            ...
            AND legal_agreement IS NOT NULL AND legal_agreement = 'accepted'
            AND privacy_agreement IS NOT NULL AND privacy_agreement = 'accepted'
        )
    )
