            ...
            <div className="flex gap-4 overflow-x-auto hide-scrollbar pb-2">
                {categories.map((category) => (
                    <button
                        ...
                        <div
                            className={`w-16 h-16 rounded-2xl border flex items-center justify-center overflow-hidden transition-all duration-300 ${
                                selectedCategory === category.name
                                    ...
                                    : "border-gray-800 bg-[#121212] group-active:border-gray-600 group-active:bg-[#1a1a1a]"
                            }`}>
                            {category.icon && (
                                <img 
                                    src={category.icon} 
                        ...
                        ${ selectedCategory === category.name ? "text-unguterang font-bold": "text-gray-400 group-active:text-gray-200 font-semibold"}`}>
                            {category.name}
                        </span>
                    </button>
                ))}
